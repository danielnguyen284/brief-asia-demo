...
import { Countries } from "./src/payload/collections/Countries";
...
import { Podcasts } from "./src/payload/collections/Podcasts";
import { MarketSnapshots } from "./src/payload/collections/MarketSnapshots";
  ...
  plugins: r2Configured
    ? [
        s3Storage({
          ...
          // Browser uploads directly to R2 via a presigned URL instead of
          // streaming through the serverless function — bypasses Vercel's
          // ~4.5MB request-body limit. Requires a CORS policy on the R2 bucket
          // allowing PUT from the site origin(s).
          clientUploads: true,
