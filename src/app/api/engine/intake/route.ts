 ...
 *
 * Contract (engine → brief-asia), mirrors src/publishers/payload-adapter.ts:
 *   POST /api/engine/intake
 *   Authorization: Bearer {BRIEFASIA_ENGINE_INTAKE_TOKEN}
...
export async function POST(request: Request): Promise<Response> {
  ...
  const expectedToken = process.env.BRIEFASIA_ENGINE_INTAKE_TOKEN;
  if (!expectedToken) {
    // Misconfiguration is a server fault, not a client one. Never silently 200.
    console.error("[engine-intake] BRIEFASIA_ENGINE_INTAKE_TOKEN is not set");
  ...
  try {
    ...
    if (sourceUrl) {
      ...
      if (existing.docs.length > 0) {
        ...
        return json({ id: doc.id, skipped: true, reason: "source already exists" }, 200);
      }
    }
    ...
    log("draft created for editorial review", { id, slug, pillar: pillarSlugStr });
